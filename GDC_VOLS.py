...
def read_from_dashboard(dashboard_url):
    ...
    inner_dashboard_data = pd.read_json(url)
    # Leave index
...
def format_table(format_frame, format_sheet, format_file_name, format_tables_names):
    ...
    wb = opxl.load_workbook(filename=format_file_name)
...
if __name__ == '__main__':
    ...
    try:
        ...
        ws = wb.create_sheet(title=report_table)
    ...
    ws['A22'] = "Строительство городских ВОЛС"
    ...
    # print_debug(2, f'{chr(0x0394)}, {datetime.datetime(year, process_month, 1).strftime("%b %Y")}')
    ...
    # TODO сделать вычисления значения в функции sum_done_events для ячейки C6
    ...
    # TODO сделать вычисления значения в функции sum_done_events для ячейки C27
